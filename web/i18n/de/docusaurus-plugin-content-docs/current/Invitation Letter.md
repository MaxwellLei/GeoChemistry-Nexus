---
sidebar_position: 4
---

# 💌 Einladungsschreiben

🔍 Wir begrüßen weitere Forscher, sich uns anzuschließen! 🤝

💪 Lassen Sie uns zusammenarbeiten, um geochemische Berechnungsmethoden und andere Plot-Vorlagen zu pflegen und zu aktualisieren! 📊

✨ Lassen Sie uns gemeinsam die Software leistungsfähiger und benutzerfreundlicher machen! 😊

:::info

Alle Mitwirkenden werden in unserer Liste der Mitwirkenden aufgeführt. Wenn Sie eine persönliche Website oder relevante Links haben, fügen wir diese mit Ihrer Erlaubnis hinzu.

:::

## 🌟 Berechnungsmethoden und funktionale Anforderungen

Im Bereich der Geochemie sind die Genauigkeit und Effizienz von Berechnungsmethoden von größter Bedeutung. **Wenn Sie fundierte Kenntnisse und Erfahrung in geochemischen Berechnungen haben oder Ihre Forschung spezifische Berechnungsmethoden umfasst, laden wir Sie herzlich ein, zu unserem Projekt beizutragen**. Ihr Fachwissen wird dazu beitragen, die Rechenleistung unserer Software zu verbessern und sicherzustellen, dass sie den Bedürfnissen der Forschungsgemeinschaft besser entspricht! Ihre Teilnahme wird auch neuen Benutzern wertvolle Unterstützung bieten und ihnen helfen, das Beste aus unseren Tools herauszuholen.

## 💻 Entwickler

Unsere Software wird mit `C#` entwickelt und basiert auf einer modernen `MVVM`-Architektur und `WPF`-Technologie, was eine hervorragende Wartbarkeit und Skalierbarkeit gewährleistet. Darüber hinaus planen wir, in Zukunft `Python` einzuführen, um bestimmte Funktionen zu implementieren, was flexiblere und vielfältigere Funktionen ermöglicht. **Wenn Sie ein erfahrener `C#`- oder `Python`-Entwickler sind, heißen wir Sie herzlich willkommen, unserem Projektteam beizutreten. Ob Sie Code beitragen, Benutzeroberflächen entwerfen oder Funktionstests durchführen möchten, Ihre Beteiligung wird unsere Software verbessern**.

![Software-Framework](https://geo-1303234197.cos.ap-hongkong.myqcloud.com/Software_Framework.png)

## 🌍 Übersetzung

Um unsere Software mehr Benutzern weltweit zugänglich zu machen, möchten wir weitere lokalisierte Sprachversionen anbieten. **Wenn Sie mehrere Sprachen beherrschen und an Lokalisierungsbemühungen teilnehmen möchten, begrüßen wir Ihren Beitrag mit Begeisterung**. Ihre harte Arbeit wird uns helfen, ein inklusiveres und zugänglicheres Produkt zu schaffen, das Forscher mit unterschiedlichem sprachlichen Hintergrund anzieht. Bitte kontaktieren Sie uns und lassen Sie uns zusammenarbeiten, um die Benutzerfreundlichkeit der Software zu verbessern!

---

## 🙏 Besonderer Dank

Siehe [Besonderer Dank](./Special%20Thanks).

## ℹ️ Über

Siehe [Über](./About).
