...
對於內建庫中沒有的圖表模板，使用者可以選擇建立自訂圖表模板。透過自訂模板並將其打包成模板包，你可以快速與其他研究人員分享。
...
現在，你可以透過選單列選擇 `檔案` -> `新建繪圖模板` 來自訂圖表模板，如下所示：
...
對於新的自訂圖表模板，主要有三個部分需要設定：

1.  **預設支援語言**：你可以從右側的選擇框中選擇內建的語言快捷選項。我們提供：簡體中文、繁體中文、美式英語、日語、俄語、韓語、德語和西班牙語。你也可以手動輸入語言代碼進行自訂設定。具體的語言代碼請參考：[語言文化名稱表](https://learn.microsoft.com/zh-cn/openspecs/windows_protocols/ms-lcid/a9eac961-e77d-41a6-90a5-ce1a8b0cdb9c)

    > 注意：在預設支援的語言中，輸入的第一個語言將作為圖表的預設語言。如果其他語言未翻譯或遇到錯誤，系統將回退到此預設語言。

2.  **圖表模板分類（層級）**：同樣，我們提供內建的快捷分類結構。此設定影響你的模板在圖表模板清單中的層級位置。

3.  **圖表模板類型**：目前支援兩種類型：**二維座標系** 和 三元圖。

完成設定後，點擊【確定】進入自訂繪圖介面。接下來，我們將操作重心放在【編輯】功能列上。點擊【編輯】後，系統將顯示二次確認對話框以編輯圖表。確認後，你將進入編輯模式，在這裡可以檢視和使用編輯功能列中的各種工具。
...
* **儲存**：儲存圖表模板。點擊後，程式預設會根據目前繪圖狀態產生相應的縮圖。
...
* **新增函數**：點擊後，新增一個預設函數 `sin(x)`，定義域範圍為 [-10, 10]。你可以在屬性面板中自訂公式。
* **復原/重做**：如果沒有建立或刪除繪圖物件，這些功能將被停用。預設情況下，歷史記錄中僅儲存最後 10 次操作。
...
下面是新增多邊形的屬性面板範例。多邊形物件有一個頂點清單。刪除頂點時會出現確認彈窗。你可以按住 `Ctrl` 鍵並左鍵點擊刪除按鈕來連續刪除頂點。
...
對於 **資料參數**，這些代表需要從資料清單中讀取哪些欄的資料。**輸入規則使用英文逗號 `,` 作為分隔符。**

**預設情況下，第一個參數可以是 `Group` 變數**。如果未新增，程式將在後台新增此變數。它的作用是在繪圖期間區分不同的資料點類別，從而影響圖例顯示。其餘參數應根據自訂底圖的需要進行定義。
...
在如下所示的位置編寫指南。我們提供簡單的工具列功能以滿足常規文件需求。你也可以點擊右側的 `Office Word` 在 Word 中開啟指南檔案以獲得更進階的格式和功能。
...
*   **資料格式**：指定有效資料讀取所需的輸入資料格式和欄標題。
*   **參考文獻**：列出用於建立底圖及其內容的參考文獻。
*   **貢獻者**：參與底圖製作人員的姓名或暱稱。甚至鼓勵你包含你的個人網站。
...
第二種方法是使用我們的主頁小元件；我們提供了一個多語言元件以方便模板本地化。
